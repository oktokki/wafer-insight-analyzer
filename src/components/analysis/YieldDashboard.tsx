...
export const YieldDashboard = ({ data }: YieldDashboardProps) => {
  ...
  if (hasEdsData) {
    ...
    performanceData = [
      { grade: 'Excellent (≥90%)', count: yieldData.filter(w => w.yieldValue >= 90).length, color: '#22c55e' },
      { grade: 'Good (80-89%)', count: yieldData.filter(w => w.yieldValue >= 80 && w.yieldValue < 90).length, color: '#3b82f6' },
      { grade: 'Fair (70-79%)', count: yieldData.filter(w => w.yieldValue >= 70 && w.yieldValue < 80).length, color: '#f59e0b' },
      { grade: 'Poor (<70%)', count: yieldData.filter(w => w.yieldValue < 70).length, color: '#ef4444' }
    ];
    ...
    kpiData = {
      ...
      trendDirection,
      processCapability: (avgYield / 85).toFixed(2) as any,
      defectRate: ((totalDies - totalPass) / totalDies * 1000000).toFixed(0) as any
    };
    ...
    yieldData = [
      { wafer: "W1", waferId: "WF001", yieldValue: 85.2, passDies: 1420, totalDies: 1668, defectRate: "14.9", efficiency: "85.2", performanceGrade: "Good" },
      { wafer: "W2", waferId: "WF002", yieldValue: 87.1, passDies: 1453, totalDies: 1668, defectRate: "12.9", efficiency: "87.1", performanceGrade: "Good" },
      { wafer: "W3", waferId: "WF003", yieldValue: 83.7, passDies: 1395, totalDies: 1668, defectRate: "16.4", efficiency: "83.7", performanceGrade: "Good" },
      { wafer: "W4", waferId: "WF004", yieldValue: 89.3, passDies: 1489, totalDies: 1668, defectRate: "10.7", efficiency: "89.3", performanceGrade: "Good" },
      { wafer: "W5", waferId: "WF005", yieldValue: 91.2, passDies: 1521, totalDies: 1668, defectRate: "8.8", efficiency: "91.2", performanceGrade: "Excellent" }
    ];
    ...
    trendData = [
      { wafer: 1, yieldValue: 85.2, cumulative: 85.2, target: 85, upperLimit: 92, lowerLimit: 78 },
      { wafer: 2, yieldValue: 87.1, cumulative: 86.1, target: 85, upperLimit: 92, lowerLimit: 78 },
      { wafer: 3, yieldValue: 83.7, cumulative: 85.3, target: 85, upperLimit: 92, lowerLimit: 78 },
      { wafer: 4, yieldValue: 89.3, cumulative: 86.3, target: 85, upperLimit: 92, lowerLimit: 78 },
      { wafer: 5, yieldValue: 91.2, cumulative: 87.3, target: 85, upperLimit: 92, lowerLimit: 78 }
    ];
    ...
    timeSeriesData = [
      { sequence: 1, yieldValue: 85.2, movingAverage: 85.2, trend: 0 },
      { sequence: 2, yieldValue: 87.1, movingAverage: 86.1, trend: 1.9 },
      { sequence: 3, yieldValue: 83.7, movingAverage: 85.3, trend: -3.4 },
      { sequence: 4, yieldValue: 89.3, movingAverage: 86.7, trend: 5.6 },
      { sequence: 5, yieldValue: 91.2, movingAverage: 88.1, trend: 1.9 }
    ];
              ...
              <ComposedChart data={yieldData}>
                ...
                <Tooltip 
                  formatter={(value, name) => [
                    name === 'yieldValue' ? `${value}%` : value,
                    name === 'yieldValue' ? 'Yield' : 'Target'
                  ]}
                />
                <Bar dataKey="yieldValue" fill="#3b82f6" />
        ...
        <CardContent>
          <ResponsiveContainer width="100%" height={350}>
            <ComposedChart data={trendData}>
              ...
              {/* Actual data */}
              <Line type="monotone" dataKey="yieldValue" stroke="#3b82f6" strokeWidth={3} name="Individual Yield" />
            ...
            {yieldData.map((wafer, index) => (
              <div key={index} className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                <div className="flex items-center space-x-4">
                  ...
                  <span className="text-sm font-medium">{wafer.yieldValue}% yield</span>
